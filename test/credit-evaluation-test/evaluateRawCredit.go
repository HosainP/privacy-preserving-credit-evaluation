...
func processCSV(filename string) error {
	...
	headers := append(records[0], "Evaluation Result")
	...
	for i, record := range records[1:] {
		...
		result := credit_evaluation.CreditEvaluation(19, 100*1000*1000+1, float64(creditScore), dti)

		newRecord := append(record, strconv.FormatFloat(result, 'f', 2, 64))
